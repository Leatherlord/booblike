...
import { Aggresive, Coward, Fury, Neutral, PlayerStrategy, Strategy } from "./strategy";


export function getCharacteristicsFromClass(class_: CharClass) {
    const name = class_.possibleNames[Math.floor(Math.random() * class_.possibleNames.length)];

    const surname = class_.possibleSurnames[Math.floor(Math.random() * class_.possibleSurnames.length)];
    ...
    const indexOfAttacks: number[] = Array.from(Array(class_.numberOfAttacks).keys());
...
export let PlayerClass: CharClass = {
    ...
    characteristicsBounds: [{s: 1, p: 1, e: 1, i: 1, a: 1}, {s: 10, p: 10, e: 10, i: 10, a: 10}],
    availableAttacks: [],
    numberOfAttacks: 9,
    strategy: {
...
export let StrongClass: CharClass = {
    ...
    possibleSurnames: ["The Fearsome", "The Sinful", "The Homosexual", "The Defiler"],
    characteristicsBounds: [{s: 6, p: 6, e: 6, i: 6, a: 6}, {s: 10, p: 5, e: 5, i: 5, a: 5}],
