...
export function attackStrength(attack: Attack, char: Character, enemy: Character) {
    const base = Math.random() * (attack.maxDamage - attack.minDamage) + attack.minDamage;
    const damage = base * (1 + 0.25 * char.characteristics.s) * (1 + 0.05 * char.level);
    const actualDamage = damage * (1 - 0.1 * enemy.characteristics.e);
    return actualDamage;
}

export function health(char: Character) {
    const HP = 10 + (0.25 * char.characteristics.s + 0.5 * char.characteristics.e) * char.level;
    return HP;
}
...
export function AttackOutcome(char: Character, enemy: Character) {
    ...
    const thresholds = [
        ...
        { threshold: phi_miss + phi_redirected + phi_self_hit, outcome: 'self-hit' },
...
function getDamage(char: Entity, enemy: Entity, attack: Attack): AttackResult {
    ...
    console.log(attackOutcome, enemy.character.healthBar, char.character.healthBar, char.character.maxHealthBar, damage)
    return {
